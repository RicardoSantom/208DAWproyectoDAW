...
class validacionFormularios {  //ELIMINA EL METODO VALIDATEDATE Y LO INCLUYE EN VALIDAR FECHA, ELIMINACION DE VALIDAR CHECKBOX Y RADIOB, MEJORA GENERAL DE RESPUESTA, INCLUSION DE PARAMETROS PREDEFINIDOS Y MEJORAS SUSTANCIALES EN ALGUNOS METODOS
     ...
     * @version 1.1 Ricardo Santiago elimina autoasignaciones de variables.
     * @since 2018-10-23
    ...
    public static function comprobarAlfabetico($cadena, $maxTamanio = 1000, $minTamanio = 1, $obligatorio = 0) {  //AÑADIDOS VALORES POR DEFECTO Y MEJORADA LA RESPUESTA
        ...
        //htmlspecialchars es una función que devuelve un string con las conversiones de caracteres especiales a entidades HTML.
...
// Si es un 1 es obligatorio, si es un 0 no lo es
     ...
     * @since 30/11/2020
     * @version 1.1 Ricardo Santiago extrae bloques de código para comprobación de condición como númérico o cadena numérica del 
     * parámetro recibido $integer a una nueva funcion implementada a continuacion de esta y de nombre comprobarNumericoMaximoMinimo.
    ...
    public static function comprobarEntero($integer, $max = PHP_INT_MAX, $min = -PHP_INT_MAX, $obligatorio = 0) {  //AÑADIDOS VALORES POR DEFECTO Y AHORA DETECTA EL 0
        ...
        if (($obligatorio == 0 && $integer != null) || ($obligatorio == 1 && empty($mensajeError))) {//COMPROBAMOS QUE SI ES OPCIONAL, NO ESTÉ VACÍO Y SI ES OBLIGATORIO QUE NO HAYA GUARDADO UN MENSAJE DE ERROR ANTERIOR (QUE EL CAMPO NO ESTÉ VACÍO)  
            ...
            $mensajeError = self::comprobarNumericoMaximoMinimo($integer, $max, $min);
            //Ricardo Santiago ha creado la funcion comprobarMaximoMinimo con los bloques de código aquí debajo comentados, el motivo es un warning de exceso de bloques
            //para una misma función que alerta NetBeans como mala práctica de legibilidad de código.
    ...
    /**
     * Funcion comprobarNumericoMaximoMinimo
     * 
     * Funcion que comprueba si el parámetro $integer no es un entero, si no lo es devuelve $mensajeError
     * indicándonos esta situación. Si no es así, es decir, si se trata de un entero numérico o una cadena
     * exclusívamente numérica, comprueba que el parámetro se encuentre dentro de los límites máximo y mínimo
     * establecidos por el lenguaje PHP para valores del tipo int.
     * 
